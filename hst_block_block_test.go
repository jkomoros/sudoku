...
func TestBlockBlockInteraction(t *testing.T) {
	options := solveTechniqueTestHelperOptions{
		targetCells:  []CellReference{{6, 3}, {6, 4}, {7, 3}, {7, 4}, {8, 3}, {8, 4}},
		pointerCells: []CellReference{{2, 3}, {2, 4}, {3, 4}, {5, 3}},
...
func TestBlockBlockInteractionAgain(t *testing.T) {
	options := solveTechniqueTestHelperOptions{
		targetCells:  []CellReference{{3, 3}, {3, 4}, {3, 5}, {5, 3}, {5, 4}, {5, 5}},
		pointerCells: []CellReference{{3, 0}, {3, 1}, {3, 6}, {3, 7}, {5, 0}, {5, 1}, {5, 6}, {5, 7}},
		targetSame:   _GROUP_BLOCK,
		targetGroup:  4,
...
func TestBlockInteractionFlipped(t *testing.T) {
	options := solveTechniqueTestHelperOptions{
		transpose:    true,
		targetCells:  []CellReference{{3, 6}, {4, 6}, {3, 7}, {4, 7}, {3, 8}, {4, 8}},
		pointerCells: []CellReference{{3, 2}, {4, 2}, {4, 3}, {3, 5}},
		targetSame:   _GROUP_BLOCK,
		targetGroup:  5,
		targetNums:   IntSlice([]int{3}),
