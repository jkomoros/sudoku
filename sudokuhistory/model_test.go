...
func TestMarkMutator(t *testing.T) {
	...
	command := model.newMarkCommand(sudoku.CellRef{Row: 0, Col: 0}, map[int]bool{1: true})

	if command != nil {
		t.Error("Got invalid command, expected nil", command)
	}

	command = model.newMarkCommand(sudoku.CellRef{Row: 0, Col: 0}, map[int]bool{1: false, 2: true, 3: false})
	...
	if !reflect.DeepEqual(command.ModifiedCells(model), sudoku.CellRefSlice{sudoku.CellRef{Row: 0, Col: 0}}) {
...
func TestNumberMutator(t *testing.T) {
	...
	command := model.newNumberCommand(sudoku.CellRef{Row: 0, Col: 0}, 0)

	if command != nil {
		t.Error("Got non-nil number command for a no op")
	}

	command = model.newNumberCommand(sudoku.CellRef{Row: 0, Col: 0}, 1)
...
func TestGroups(t *testing.T) {
	...
	model.SetNumber(sudoku.CellRef{Row: 0, Col: 0}, 1)

	if model.grid.Cell(0, 0).Number() != 1 {
		t.Fatal("Setting number outside group didn't set number")
	}

	model.SetMarks(sudoku.CellRef{Row: 0, Col: 1}, map[int]bool{3: true})
	...
	model.SetNumber(sudoku.CellRef{Row: 0, Col: 2}, 1)
	model.SetMarks(sudoku.CellRef{Row: 0, Col: 3}, map[int]bool{3: true})
...
func TestUndoRedo(t *testing.T) {
	...
	rememberedModfiedCells = append(rememberedModfiedCells, sudoku.CellRefSlice{sudoku.CellRef{Row: 0, Col: 0}})

	model.SetNumber(sudoku.CellRef{Row: 0, Col: 1}, 2)

	rememberedStates = append(rememberedStates, model.grid.Diagram(true))
	rememberedModfiedCells = append(rememberedModfiedCells, sudoku.CellRefSlice{sudoku.CellRef{Row: 0, Col: 1}})
	...
	model.SetMarks(sudoku.CellRef{Row: 0, Col: 2}, map[int]bool{3: true, 4: true})

	rememberedStates = append(rememberedStates, model.grid.Diagram(true))
	rememberedModfiedCells = append(rememberedModfiedCells, sudoku.CellRefSlice{sudoku.CellRef{Row: 0, Col: 2}})

	model.SetMarks(sudoku.CellRef{Row: 0, Col: 2}, map[int]bool{1: true, 4: false})
	...
	model.SetNumber(sudoku.CellRef{Row: 2, Col: 0}, 3)
