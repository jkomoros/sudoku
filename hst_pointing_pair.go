	...
	for _, i := range rand.Perm(DIM) {
		...
		for _, num := range rand.Perm(DIM) {
			...
			if (self.groupType == _GROUP_ROW && cells.SameRow()) || (self.groupType == _GROUP_COL && cells.SameCol()) {
				...
				var step *SolveStep
				if self.groupType == _GROUP_ROW {
					step = &SolveStep{self, grid.Row(cells.Row()).RemoveCells(block), []int{num + 1}, cells, nil, nil, nil}
				} else {
					step = &SolveStep{self, grid.Col(cells.Col()).RemoveCells(block), []int{num + 1}, cells, nil, nil, nil}
				}
